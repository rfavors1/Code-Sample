...
<title>Search Accounts by Deletion Date - Admin</title>
<style type="text/css">
<!--
body {
...
<div id="centeredmenu">
  ...
  <div class="contentcontainer">
    <p align="center">&nbsp;</p>
  <strong><h2>Date Search:</h2></strong>
  <?php
  echo("<p><strong>" . $_POST['dateafrom'] . " - " . $_POST['dateato'] . "</p>");
...
$DateFrom = $_POST['dateafrom'];
$DateTo = $_POST['dateato'];
$newdatefrom = substr($DateFrom,6,4) . "-" . substr($DateFrom,0,2) . "-" . substr($DateFrom,3,2);
$newdateto = substr($DateTo,6,4) . "-" . substr($DateTo,0,2) . "-" . substr($DateTo,3,2);
...
<p align='center'>&nbsp;</p>");
	...
	while($row)
  {
	  $Deletedate = $row['DateDelete'];
	  $i++;
	  if ((strtotime($Deletedate) >= strtotime($newdatefrom)) and (strtotime($Deletedate) <= strtotime($newdateto))) {
