...
<title>Account's Requests</title>
<style type="text/css">
<!--
body {
...
$namerep = $_SESSION['repa'];
echo("<p><h3><strong>" . $namerep . "'s Requests</strong></h3></p>");

mysql_select_db(reg_ister_13_info);

$query="SELECT * FROM $TableName WHERE Email = '$User'";
...
if (!$row) {
		echo("<p align='center'>&nbsp;</p><div align='center'>The user does not have any requests for assistance at this time.<p></p><p></p></div>
          <p align='center'>&nbsp;</p>
          <p align='center'>&nbsp;</p>
		    <p align='center'>&nbsp;</p>
...
<p align='center'>&nbsp;</p>");
	...
	$i = "0";
echo("<center><strong>Here is a list of this user's requests:</strong><p></p></center>");
while($row)
  {
$ID=$row['ID'];
$Type=$row['Type'];
...
if(!(($Type == 'Telephone') or ($Type == 'Mortgage/Rent') or ($Type == 'Electricity/Gas'))) {
	...
	<tr bgcolor='#990000'>
	<td  colspan='4'><strong><font color='#FFFFFF'>Date Requested: " . $newdate . "</font></strong>
	</td>
	</tr>
   <tr bgcolor='#CCCCCC'>
     <td width='10%'><strong>ID:</strong> " . $ID . "</td>
     <td width='30%'><strong>Type:</strong> " . $Type . "</td>
	  <td width='25%'><strong>Status:</strong><font color='#FF0000'> "); 
	    $today = strtotime(date("Y-m-d"));
	$datec = strtotime($row['DateExpired']);
	$Expired='no';
	if (($Status == 'Pending') or ($Status == 'Fulfilled')) {
		 echo($Status . "</font></td>");
	} else {
	if ($datec > $today) {
		 echo($Status . "</font></td>");
	} else {
		echo("Expired</font></td>");
		$Expired = 'yes';
	...
	if ($Expired == 'yes') {
	 ...
	 <a href='editrequest.php?ID=" . $ID . "'><font color='990000'><u>EDIT</u></font></a>&nbsp;&nbsp;&nbsp;<a href='deleterequest.php?ID=" . $ID . "'><font color='990000'><u>DELETE</u></font></a></td>");
	 } elseif (($Status == 'Pending')) {
		 echo("<td width='35%' align='right'><a href='changestatus.php?ID=" . $ID . "'><font color='990000'><u>CHANGE STATUS</u></font></a></td>");
	 }  elseif (($Status == 'Fulfilled'))  {
		 echo("<td width='35%' align='right'>&nbsp;</td>");
	 } 
   echo("</tr>
   <tr>
     <td valign='top' colspan='3'><strong>Contact Name:</strong> " . $ContactName . "<br>
  ...
  echo($Status . "</font></td>");
   ...
   <tr>
     <td colspan='4' valign='top'><strong>Request:</strong> " . $Request . "</td>
   </tr>
 </table><hr border='1' color='#990000' width='50%' align='center'><p></p>");
}
$i++;
