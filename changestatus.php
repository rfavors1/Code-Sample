...
{
var status=document.forms["StatusForm"]["Status"].value;
 if (status.length == 0) {
	 alert('Please enter a status');
	 document.forms["StatusForm"]["Status"].focus();
    return false;
 }
}
...
<div id="centeredmenu">
  ...
  <p>&nbsp;</p>
  <h2><strong>Change Status</strong></h2>
  <p></p>
 <form action="submitstatus.php" method="post" name="StatusForm" id="StatusForm" onsubmit="return ValidateForm();">
  <?php 
...
if (($row['Email'] != $User) or ($row['Status'] == 'Fulfilled') or ($row['Status'] == 'Unfulfilled')) {
echo("<script>
<!--
location.replace('updated.php?page=error');
-->
...
$Status=$row['Status'];
  ...
  <td width='50%'><select name='Status' size='1' required='required'>
  ...
  <tr><td colspan='2'>ID: " . $ID ."</td><input type='hidden' name='ID' id='ID' size='30' required='required' value='" . $ID . "'/> 
  </tr> 
...
<p></p>
			...
			<center><input type='submit' name='singup' id='singup' value='Change Status' /></form>&nbsp;&nbsp;<a href='myrequests.php'><font color='#990000'><u>GO BACK</u></font></a>
");
}
}
?>
